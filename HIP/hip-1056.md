...
message MapChangeValue {
    oneof value_choice {
        ...
        */
        ...
        /**
         * A set of EVM storage slot values with associated indexes and keys.
         */
        repeated StorageSlot storage_slot_values = 9;
...
represented as a binary merkle tree with 8 sub-trees at depth 2 which create the block proof and tree when hashed to
...
3. (010) Merkle root of the consensus header tree - root hash of combined round and event header items merkle tree
...
7. (110) Merkle root of trace data outputs tree - root hash of trce data item merkle tree
8. (111) `NULL` - empty leaf node padded to create balanced tree and may be assigned in the future
...
message BlockProof {
    ...
    */
    ...
    /**
     * The hash of the merkle tree root for the network consensus headers comprised of Round and Event headers of this block.
     */
    bytes consensus_headers_root_hash = 4;

    /**
     * The hash of the merkle tree root for the user transactions submitted to the network in this block.
     */
    bytes transaction_root_hash = 5;

    /**
     * The hash of the merkle tree root for the outputs of transactions in this block comprised of results and output items.
     */
    bytes result_output_root_hash = 6;

    /**
     * The hash of the merkle tree root for the state changes as a result of transactions in this block.
     */
    bytes state_changes_root_hash = 7;

    /**
     * The hash of the merkle tree root for the trace data user transactions submitted to the network in this block.
     */
    bytes trace_data_root_hash = 8;
    ...
    */
    repeated MerkleSiblingHash sibling_hashes = 10;
}
...
To this point the `TraceData` would initially contain smart contracts data that support traceability information (e.g.
