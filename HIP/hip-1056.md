...
The publicly consumable output from the consensus node are 2 streams - the event stream (ordered collection of events
...
for hashing purposes only. Its Merkle Mountain Top is a fixed 16-position
sub-tree (see [HIP-1424](./hip-1424-block-stream-hashing.md)) whose defined
positions include the merkle tree of the previous block, the all-blocks
history tree, and the state merkle tree, alongside the block's own item
sub-trees. Hash computation for the block `N` merkle tree reuses the hash
...
> Each block will contain its own Block Proof, which provides proof of consensus agreement on the content of the block.
...
message BlockItem {
    ...
    oneof item {
        ...
        /**
         * A single signed transaction, transmitted as a byte array.
         */
...
All items in the block other than the final `BlockProof` and `BlockFooter` are assigned to one of five streaming
Merkle hashers. This separation is important because items are hashed into five independent sub-merkle-trees that are
combined to form the final block root hash.
...
| `signed_transaction` (field 4, raw bytes) | `inputTreeHasher` |
| `BlockHeader`, `TransactionResult`, `TransactionOutput`, `RecordFileItem` | `outputTreeHasher` |
| `StateChanges` | `stateChangesHasher` |
| `TraceData` | `traceDataHasher` |
| `FilteredSingleItem`, `RedactedItem` | Whichever hasher the item's own `tree` field (a `SubMerkleTree` value) names — the hash stub takes the place of the original item's hash at that hasher's stream position |
| `BlockProof`, `BlockFooter` | _(not hashed)_ |
...
the `CryptoTransfer` transaction will be described by its `SignedTransaction`,
`TransactionResult`, and `TransactionOutput` and `BlockItem`s. The `CryptoCreate`
transaction will also be described by its own `SignedTransaction`,
...
The distinction between a **logical transaction** in the block stream and its
**Hiero transactional unit** is especially significant for
[batch transactions](https://hips.hedera.com/hip/hip-551). The `AtomicBatch`
transaction lets a Hiero user group arbitrary HAPI transactions into an
all-or-nothing unit. None of the HAPI transactions in the group have their own
`StateChanges` block item---only the `AtomicBatch` has a `StateChanges` item.
This item includes _either_ the effects of executing every transaction in
the group with final status `SUCCESS`; _or_ just the effects of paying fees for
whatever transactions in the group were attempted before getting a status other
than `SUCCESS`. The logical transactions inside the batch are legible in the
block stream because their "input" appears as a `SignedTransaction` item in the
batch and their "output" appears as a `TransactionResult` item along with any
`TransactionOutput` or `TraceData` needed for extra detail. But a `StateChanges`
item always represents the boundary of a Hiero transactional unit, and never the
boundary of a logical transaction.
 ...
 * Fields numbered 20 and above MUST be numbered as follows.
    * Calculate the category number as N modulo 20, where N is the actual field number.
       ...
       * 1, 2 - Requires specific handling; not forward compatible
       * 3 - Consensus Headers
       * 4 - Inputs
       * 5 - Outputs
       * 6 - State Changes
       * 7 - Trace Data
       * 8-15 - Extension 0 through Extension 7 (eight slots, matching the eight reserved future positions in the
         `SubMerkleTree` enum)
       * 16-18 - Reserved
 ...
 * The "BlockTrailer" is field 20 (20 modulo 20 is 0, so it is Not Hashed).
...
The block root hash has two children: a leaf carrying the block's consensus timestamp, and the root of a fixed
**16-position Merkle Mountain Top**. All 16 positions — the first 8 active, and slots 9–16 (`FUTURE_1` through
`FUTURE_8` in the `SubMerkleTree` enum) reserved for future use — are always fed into the same streaming Merkle tree
algorithm, joined only by two-child (`0x02`) internal nodes at every level. A reserved or otherwise-empty position
contributes the canonical empty-tree value `hash(0x00)`; it is never omitted, and its parent is never hashed as a
single-child (`0x01`) node — this uniformity keeps every position's proof path at a permanently fixed depth. The
hashing algorithm, domain-separation scheme, and full rationale for this design are specified in
[HIP-1424](./hip-1424-block-stream-hashing.md).
...
| `FUTURE_1`–`FUTURE_8` (9–16) | 8–15 | _(reserved)_ | — | `hash(0x00)` until a future HIP assigns a sub-tree; never omitted |
...
4. **Check the acceptance threshold.** A block is accepted when the count of valid signatures meets a strong
   minority (more than one-third) of the active node roster at the time of the block. For blocks predating
   verifiable node-stake history, this is `validatedCount >= floor(rosterSize / 3) + 1`; once node-stake data is
   available for the block's timestamp, the threshold is instead stake-weighted: `validatedStake >=
   ceil(totalStake / 3)`. This matches the threshold used by the WRB validation CLI.
...
produce an aggregate signature for a block. The `StateProof.paths` field (a list of `MerklePath`, each carrying
`siblings`) will be empty in all cases other than this rare case where it must be used to create a merkle proof for
...
For historical wrapped record blocks, `start_of_block_state_root_hash` is set to the canonical empty-tree value
`hash(0x00)` (not literal zero bytes — see [HIP-1424](./hip-1424-block-stream-hashing.md)) because the state hash was
not recorded in the original record stream at the time those blocks were created.
...
message BlockFooter {
     ...
     * SHALL be the canonical empty-tree value hash(0x00) for the stream-starting block.
     */
     ...
     * Set to the canonical empty-tree value hash(0x00) for historical wrapped record blocks.
     */
...
The following diagram illustrates the full 16-position tree, how blocks are chained and signed, and how one of the
streaming sub-trees is actually built from individual items.

![Block Stream Merkle](../assets/hip-1056/block-stream-merkle.svg)

The input and output item merkle trees are built with the streaming binary Merkle tree algorithm defined in
[HIP-1424](./hip-1424-block-stream-hashing.md): items are folded in incrementally, in stream order, using an
incremental carry-based algorithm that produces a valid root for any item count without padding to a power of two and
without ever needing a null or empty-byte placeholder leaf. This same incremental nature allows the consensus node or
a block validator to calculate the input and output tree merkle hashes as items are received in a streaming way.
...
### Three Forms of BlockProof (Direct Signature, Chained State Proof, Historical RSA Proof)
The block stream supports three forms of `BlockProof` to handle different situations (see
[§Block Proof Forms](#block-proof-forms) above for the full specification):

- **`TssSignedBlockProof`** (direct proof): The consensus node computes a TSS-BLS aggregate signature directly over
  the block's root hash. This is the preferred form and is used immediately after genesis and network cutover.

- **`StateProof`** (chained proof): If the consensus node cannot obtain sufficient signature fragments for a block
  in time, it includes a Merkle path linking the block's root hash to a later block that *does* have a valid
  aggregate signature. This allows the block stream to continue without blocking, at the cost of a larger proof size.

- **`SignedRecordFileProof`** (historical proof): used only for blocks produced before the block stream cutover,
  carrying the original RSA signatures from the historical record file rather than a TSS signature (see
  [HIP-1427](https://hips.hedera.com/hip/hip-1427)).

The first two forms alone were considered insufficient for live blocks: without a chained fallback, a single block
missing enough signature fragments in time would stall block production entirely. This dual-form design for live
blocks prevents signature-fragment delays from stalling block production while maintaining full cryptographic
assurance; the third form separately extends the same `BlockProof` interface to cover the pre-cutover historical
ledger.
