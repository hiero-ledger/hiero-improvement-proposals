...
> **Implementation status:** hiero-block-node's block-verification hasher does not yet implement the
> `FilteredSingleItem`/`RedactedItem` tree-field routing above — encountering either item type currently fails block
> verification. The `tree` field's entire purpose is to enable this routing, so the table above states the intended,
> normative behavior; closing this gap is an implementation task, not an open design question.
