...
1. Stores partial WRB hash data for every record file created during a full release starting 2 releases before cutover.
2. Reads `jumpstart.bin` from a configurable path at startup 1 release before cutover.
...
5. Sets the `previous_block_root_hash` field of the next block's `BlockFooter` and the same value used in the WRB hash
   calculation from the Jumpstart file.
6. Catches up to "current" block before resuming regular operation.
7. Continues WRB hashing with complete information and maintains the previous block root hash and all blocks hashes
   tree from that block forward.
...
node needs two things from history: The hash of the last block, and the Merkle tree state that accumulates all bloc
hashes. These two items are pre-computed by the wrapping process so that the processing of the history can be completed
without interrupting continued operation of the network. The Consensus Node will load the file at startup, use the
block number to find which block in the previous 30 days corresponds to the hashes in the file, compute the WRB root
hashes for every block after that up to the current while maintaining the hash chain and hash tree, and continue
maintaining this data in state from that point forward. This ensures that the first live block after cutover will be
correctly linked to the full historical chain and will have a correct values for the all blocks merkle tree.
