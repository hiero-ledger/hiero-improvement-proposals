...
status: Approved
last-call-date-time: 2026-04-27T07:00:00Z
created: 2026-03-11
updated: 2026-05-27
requires: 1056, 1424
---
...
all historical record files produced from the network stream starting point through the cutover point must be converted into the block
...
- A **jumpstart contents** payload that the consensus node loads at first startup after the upgrade to resume block
hashing from the correct cryptographic state.
- A **verified collection of wrapped record blocks** that Tier-1 block nodes use to backfill the complete recorded
network history from the stream starting point.
...
[HIP-1056](https://hips.hedera.com/hip/hip-1056) introduced block streams as the unified, state-proof-carrying data
format for the Hiero network. [HIP-1193](https://hips.hedera.com/hip/hip-1193) defines the upgrade boundary at which
consensus nodes begin producing block streams, ensuring that all future history is available in the new format.
This HIP completes the picture: it brings the full historical record file corpus — every transaction from the network
stream starting point through the cutover — into block stream format, giving participants a single, consistent API
surface across all of network history.

Achieving that continuity requires solving two concrete initialization challenges:

**1 — Consensus node initialization.**
...
all prior block root hashes in order. For the first live block the consensus node produces after the cutover, the
"previous block" is the last wrapped record file block, and the Historical block hash Merkle tree must already
incorporate all prior block hashes. Pre-computing these values from the full historical record file corpus allows
the consensus node to produce a correctly-linked first block with no additional startup delay.

**2 — Block node historical data.**
Block nodes are the long-term authoritative store of the block stream. A block node that starts after the cutover
has no mechanism to serve pre-cutover blocks to clients. Converting the historical record files into block stream
format allows block nodes to serve a seamless, unbroken history from the network stream starting point to the present,
through the unified block stream API.

This HIP defines the one-time offline process that solves both considerations, enabling a complete, independently
verifiable block stream ecosystem that spans the full recorded history of the network.
...
### Why separate jumpstart contents rather than deriving state from the wrapped record blocks
...
each with its own internal structure and hashing process. The wrapping tool handles all three versions transparently
during conversion, ensuring that a single consistent hashing process can be applied to the wrapped blocks.

## User Stories

### Consensus Node Operator

1. **As a consensus node operator**, I need the jumpstart contents to be generated and deployed to each consensus node a
   full release before the cutover so that I can catch up to and maintain the current previous block root hash and
   Historical block hash Merkle tree root prior to the cutover upgrade, maintaining unbroken cryptographic continuity
   with the historical record file chain.

2. **As a consensus node operator**, I want the jumpstart contents format to be well-defined and small so that it can be
   validated, distributed, and loaded quickly during the upgrade process without adding meaningful delay to the
   maintenance window.

### Block Node Operator

3. **As a Tier-1 block node operator**, I want the complete collection of deduplicated wrapped record blocks to be
   publicly available so that I can choose how much history to ingest and verify, and serve historical blocks to
   clients from the network stream starting point up to whatever depth my infrastructure retains.
...
![Five-Phase Conversion Pipeline](../assets/hip-1427/pipeline.svg)
...
to `jumpstart.bin`, then distributed to each consensus node via a `FileUpdate` transaction to update file `0.121.0`
ahead of the cutover upgrade. The contents are signed and agreed upon by network operators before distribution.
...
nohup java --enable-native-access=ALL-UNNAMED \
  -jar tools.jar days download-days-v3 \
...
Conversion is estimated to take several days or more on fast hardware.
...
![Four-Stage Wrap Pipeline](../assets/hip-1427/wrap-pipeline.svg)
...
Before the wrapped record blocks are accepted for production use, they SHALL be verified using the `blocks validate` command.
...
#### 5.3 Public availability for independent verification

Upon completion of the conversion and validation pipeline, the following artifacts SHALL be made publicly available
so that any party can independently verify the integrity of the conversion:

- The full set of wrapped record block (WRB) zip archives
- The `addressBookHistory.json` used during conversion

The hosting location (e.g., a public GCP bucket or CDN) will be announced as part of the block node launch
operations plan. Independent verification using the `blocks validate` command (see §5.2) is expected and encouraged
before any party relies on the wrapped record blocks for production use.

---

### Node Integration Coordination
#### Consensus Node Integration

The jumpstart contents are signed and agreed upon by network operators before distribution and are deployed to
consensus nodes via a `FileUpdate` transaction to file `0.121.0` along with values for the following configurations.
...
This on-chain distribution mechanism means the
contents carry the same governance weight as any other network file update.
...
additive: a new binary artifact (the jumpstart contents) and a new set of block stream files are produced.
...
The jumpstart contents' 48-byte hash and the block registry (`blockStreamBlockHashes.bin`) form a complete,
verifiable record of the SHA-384 root hashes of all historical blocks. Any tampering with a wrapped record block
would change its hash, breaking the chain encoded in all subsequent blocks and producing a mismatch with the
jumpstart contents. The `blocks validate` command detects such tampering before any artifact is deployed.
...
### Jumpstart Contents Distribution

The jumpstart contents are a critical input to the first live block. The contents are signed and agreed upon by
network operators before distribution and are deployed on-chain via a `FileUpdate` transaction to file `0.121.0`,
providing the same integrity guarantees as any other governance-controlled network file. Independent parties can
verify the jumpstart contents against the publicly available WRB archives (§5.3) before the cutover upgrade
is applied. Tampered jumpstart contents would cause consensus nodes to diverge on the Historical block hash Merkle
tree root in the first live block, preventing threshold signature aggregation — a mismatch the consensus node's
hash verification step (see Consensus Node Integration) is designed to detect.

## How to Teach This

### For consensus node engineers

The jumpstart contents are the handshake between the historical wrapping process and the live block producer. The
consensus node needs two things from history: the hash of the last block and the Merkle tree state that accumulates
all block hashes. These two items are pre-computed by the wrapping process so that history can be processed without
interrupting continued network operation.

At startup, the consensus node loads the jumpstart contents and uses the block number to locate the corresponding block
within the previous 30 days. It then computes WRB root hashes for every block from that point up to current, maintaining
the hash chain and hash tree throughout. From that point forward, it continues maintaining both the previous block root
hash and the all-blocks hash tree in state. This ensures the first live block after cutover is correctly linked to the
full historical chain with correct values for the all-blocks Merkle tree.
...
- [Cutover Process](https://github.com/hiero-ledger/hiero-block-node/blob/main/docs/block-node/Cutover-Process.md)
