...
an `AtomicBatch` for FOK in our setting because hook executions are disabled in batch transactions.)
...
This HIP allows some transactions that would previous fail to succeed (at a higher price). This is _conceivably_ a breaking
change only for clients relying on failure semantics rather than pricing, although this may require a contrived scenario.
