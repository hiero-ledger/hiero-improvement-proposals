...
So it makes sense to reduce the intrinsic gas cost of their execution. We propose adding another network property to
give
...
A hook is an extension of its owning entity, in the style of
Ethereum's [account abstraction](https://ethereum.org/en/roadmap/account-abstraction/)
...
The implementation will support these new REST endpoints:
...
`hook.id` parameter in the `links.next` URL uses the format `hook.id=lt:5` for pagination.
...
Returns the storage state for a hook:
...
2. All `pre_tx_allowance_hook` and `pre_post_tx_allowance_hook` calls in the fungible `transfers` of the
   `tokenTransfers`
...
3. All `pre_tx_sender_allowance_hook`, `pre_post_sender_tx_allowance_hook`, `pre_tx_receiver_allowance_hook`, and
   ...
   order they appear in the `CryptoTransferTransactionBody`. (_Note:_ if both sender and receiver hooks are present,
   then
...
- [Hedera public mirror node
  `getAccount` API](https://mainnet-public.mirrornode.hedera.com/api/v1/docs/#/accounts/getAccount)
