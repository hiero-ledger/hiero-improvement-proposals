...
While in the hook frame, the following EVM instructions are prohibited:
- `CALLCODE` will revert the hook frame and the transaction fails with `REJECTED_BY_ACCOUNT_ALLOWANCE_HOOK`
- `DELEGATECALL` is allowed to system contracts or native entities; all other targets will revert the hook frame and 
the transaction fails with `REJECTED_BY_ACCOUNT_ALLOWANCE_HOOK`
- `SELFDESTRUCT` will revert the hook frame and the transaction fails with `REJECTED_BY_ACCOUNT_ALLOWANCE_HOOK`
