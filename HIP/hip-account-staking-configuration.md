...
requested-by: LimeChain / Entente
...
Concretely, the Entente mainnet custody contract (`Trader`) locks user HBAR and wants that balance to
...
**Map one function to one protobuf field, not one combined "update" call.** The underlying
`CryptoUpdate` staking fields are a `oneof staked_id { staked_account_id, staked_node_id }` plus a
wrapped `decline_reward` whose "unset" state means "leave unchanged". Rather than a single
`updateStakingConfig(nodeId, decline)` that conflates target and reward and cannot express
"unchanged", this HIP exposes orthogonal setters — `stakeToNode`, `stakeToAccount`, `unstake`,
`setDeclineReward` — each mapping to exactly one field. Callers that need to set several fields
atomically simply issue several calls in one EVM transaction: they either all take effect or the
transaction reverts, so atomicity is preserved without overloading a single selector. This also
matches how HIP-906 kept `hbarAllowance` and `hbarApprove` as distinct, single-purpose calls.
...
Document the four operations alongside the existing HIP-906 HBAR-allowance examples, since they share
...
contract dispatch for the four functions on both interfaces, the authorization wiring to the existing
...
admin key, and a rejected cross-account call). A downstream consumer — the Entente `Trader` custody
...
contract account, with no `vm.etch` or bytecode substitution.

## Rejected Ideas

- **A single `updateStakingConfig(int64 nodeId, bool decline)` selector.** Conflates the staking
  target and the reward flag, cannot express the protobuf "leave `decline_reward` unchanged"
  semantics, and offers no way to stake to an account rather than a node. The orthogonal setters map
  cleanly to the protobuf fields and compose atomically within one transaction. (An interface of this
  exact shape was prototyped in the Entente contracts against a *non-existent* `0x16a` selector and
  had to be removed — it compiled and unit-tested only because a mock was `vm.etch`ed at `0x16a`, and
  would have reverted on any real network. That failure is the direct motivation for specifying the
  real surface here.)
- **A new HAPI transaction / `HederaFunctionality`.** Unnecessary: the staking logic already exists
  in `CryptoUpdate`; a system-contract dispatch reuses it with far less surface and no protobuf churn.
- **Configuring staking only at `ContractCreate` and treating it as immutable.** Works for a
  set-once policy but cannot follow node retirement or governed rebalancing, which is exactly the gap
  motivating this HIP.
- **Requiring an admin key on the contract so a `ContractUpdate` can change staking.** Hands a
  ledger-level superuser (re-key / delete) to whoever holds that key, over a funds-custody contract,
  bypassing the contract's own governance — a strictly worse security posture than a keyless,
  authorization-gated EVM call.

## Open Issues

- Final gas schedule for the four functions (expected comparable to the HIP-906 `hbarApprove` call).
