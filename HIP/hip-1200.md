...
The hinTS scheme provides a constant-size signature that can be cheaply verified (including by an EVM smart
contract via gas-efficient precompiles). It achieves this by pairing each BLS aggregate signature with a
lightweight SNARK proof attesting that the signature is a valid aggregation of partial signatures from
BLS keys in the network address book for nodes with a threshold amount of weight. That is, hinTS achieves
two properties that are crucial for many Hiero use cases:
