...
The hinTS scheme provides a constant-size signature that can be cheaply verified (by an EVM smart contract). 
It achieves this by pairing each BLS aggregate signature with a lightweight SNARK proof attesting that the 
...
Verifiers will verify Hiero hinTS signatures under a sequence of **verification keys** 
...
will publish a recursive SNARK proof in the block stream that proves the new address book's 
verification key belongs to the network's **chain of trust** -- a witness to this recursive
SNARK proof is a set of ed25519 signatures from members of the prior address book, representing 
a threshold amount of weight.

## Motivation
Inter-ledger communication between Hiero networks and EVM chains requires an efficient threshold
signature scheme, whose cryptographic requirements has robust support in EVM smart contracts. 
Specifically, no matter the size of the Hiero network or the distribution of weights, 
the scheme must yield constant-size signatures that require only constant-time verification work. 
Anything less makes inter-ledger communication impractical.
...
(SNARK) based on honest nodes contributing signatures to address book. These signatures are done
using Schnorr private keys whose public keys are gossiped to the network. The message used to
gossip a Schnorr public key is,
```
...
The SNARK  represents the network membership as a **TSS address book** made up of
`(nodeId, proofKey, weight)` triples; where the `proofKey`s are the Schnorr keys gossiped as
above; and the `nodeId` and `weight` fields are identical to those in a current or candidate
consensus roster.
...
address book**. From that point forward, the SHA-384 hash of the genesis address book is the
 **ledger id**.
```
...
book, then gossiping their Schnorr signatures on the hash of that address book after being
_concatenated with arbitrary metadata that should be trusted for that address book_. The
message used to gossip these signatures is,
```
...
valid signatures as inputs to the SNARK; compute the proof; and then gossip their vote
for that proof as the evidence the roster transition is trustworthy.
...
work together to construct the hinTS scheme for that address book. The first step
is to gossip a special, cryptographically "extended" forms of the public BLS keys
they have generated for their individual use.
```
...
ledger id and the genesis hinTS `VK` for at least 1/3 of the weight in the genesis
roster.
...
message BlockProof {
  ...
  oneof {
    ...
    /**
     * If this is not the genesis block, an uncompressed SNARK proving the
     * chain of trust from the ledger id to this hinTS verification key;
     * approximately 1.5MB.
     */
    bytes wraps_proof = 8;
    /**
     * If this is not the genesis block, a ZK-compressed SNARK proving the
     * chain of trust from the ledger id to this hinTS verification key;
     * approximately 1KB.
     */
