...
Consensus Nodes will push block stream data to Block Nodes. This avoids the traffic and implications of Block Nodes
querying a Consensus and allows Consensus Nodes to control their data rate. As such a block node must be ready and
available to accept connections that push the block stream to as blocks are agreed upon by the consensus network.
In this mode the Consensus Node or other block stream clients act as the Publisher feeding the Block Node data.
...
formats, cryptographic proofs, and streaming protocols. By decoupling data access from the consensus layer, Block
