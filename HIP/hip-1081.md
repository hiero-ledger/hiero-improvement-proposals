...
As outlined above, Hiero Block Nodes serve two primary roles:
- **Network Services**  – Ingesting, verifying, and storing output data from Hiero consensus nodes.
- **Data Access** – Providing value-added services that make network data easily accessible to users and applications.

To support these distinct functions, Block Nodes are organized into two operational tiers:
...
he broader community. They receive block streams from Tier 1 nodes and may offer both free and monetized data services.
...
The remainder of this Specification section is organized according to the two core roles to clearly delineate
protocol-level responsibilities from service-layer innovations.
...
> Block node developers are encouraged to support a modular, pluggable architecture that allows third party node
operators to extend and innovate on top of the Hiero specified APIs. This will also encourage light weight and scalable
block nodes for community operators.
