...
This HIP introduces a new node type, the **Block Node**, designed to parse block streams, store historical blocks,
state and provide APIs for decentralized querying and verification of network data. Block Nodes act as intermediaries
between consensus nodes and mirror nodes, offering verifiable network state and transaction information. They also
enable a new node economy by allowing independent operators to run Block Nodes and provide value-added services, such
as filtered block streams, aggregated data insights.

Block Nodes feature a robust API, enabling consumers to access either a filtered subset or the full stream of data.
Block Nodes can innovate on payment -- they can be commercial and have connectivity rules / rate limits / SLAs for
customers. They can require payment for serving blocks, or they can serve blocks for free.

In addition to replacing the cloud storage buckets currently in use, Block Nodes will:
...
Mirror nodes will also be updated to be able to retrieve block streams directly from Block Nodes.
...
The current network architecture relies heavily on centralized cloud storage for consensus node output data (e.g.
events, records, state, sidecars and signatures.) Since 2019 mirror nodes and other indexers have utilized this format
to support DApps.

However, this approach has introduced several challenges such as:
- **High costs**: Accessing and parsing data is expensive.
- **Limited decentralization**: Network data storage is concentrated in a few cloud providers.
- **Limited state APIs**: There are no APIs for live or historical state queries.

The primary motivations is to decentralize network data storage, reducing reliance on centralized providers and
enabling anyone to run a Block Node.

Additional challenges that Block Nodes aim to address include:

- Consensus nodes relying on other consensus nodes to catch up on the latest Hashgraph information.
- The absence of proof-of-state APIs from consensus nodes.
- The inability of community members to easily run independent nodes with complete state and transaction information to
verify consensus outputs.


## Rationale

The Block Node aims to increase data availability, decentralization and client diversity.

To achieve these goals, the Block Node will use **gRPC over HTTP/2** for communication.
This protocol was chosen for its:
- High-performance streaming.
- Cross-platform support.
- Compatibility with protocol buffers, ensuring clean and extensible APIs.

This approach aligns with the communication protocol used by consensus nodes but differs from other Web3 nodes, which
often use JSON-RPC APIs over HTTP. In the future, additional support such as WebSocket (WS), REST APIs, or GraphQL may
be explored based on community needs.
