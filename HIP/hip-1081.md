...
10. As a Block Node Operator, I want to expose a publish endpoint for upstream publishers, so that Consensus Nodes or
...
> When the publisher is a consensus node it is important to expose error cases that suggest a blocks node is falling
