...
- **Hiero Consensus Node Operators**: Enterprise-affiliated managers of consensus nodes responsible for network stability.
- **Independent Community Node Operators**: Independent entities running nodes to support decentralization.
- **Mirror Node Operators**: Managers of mirror nodes focused on rich HAPI data indexing and querying services for DApps.
- **Block Node Operators**: Managers of Block Nodes focused on raw block stream data indexing and querying services.
- **Custom dApps**: Applications that rely on network data for high TPS or low-trust use cases.
...
1. As a consensus node operator I want to publish a block stream to one or more block nodes so that I am not required
...
1. As a custom dApp I want to subscribe to an unfiltered and verified block stream from a block node.
2. As a custom dApp I want to subscribe to a filtered and verified block stream from a block node.
3. As a custom dApp I want an API to request a copy of the periodic historical state snapshots maintained and made
available by a block node.
4. As a custom dApp I want an API to request a state proof for any item in state, as of an arbitrary historical block,
from a block node. 
5. As a custom dApp I want an API to request a state proof for any item in state, as of the most recent block, from a
block node.
