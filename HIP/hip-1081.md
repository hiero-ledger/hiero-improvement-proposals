...
working-group: >-
    ...
    Georgi Lazarov <@georgi-l95>, Nick Poorman <@nickpoorman>
...
This HIP introduces Block Nodes, a new node type in the Hiero network architecture that enhances how network output
data—such as blocks and state—is processed, verified, stored, and distributed to downstream services.
Block Nodes receive block streams from consensus nodes (or other block stream producers), maintain network state,
and enable critical capabilities including state proofs, block item proofs, and node sync and bootstrap services.
...
maintaining network state, Block Nodes offload key data management functions from consensus nodes, improving
scalability and operational efficiency.

In addition to their core network functions, Block Nodes expose robust APIs to provide filtered or full data streams,
enable random access, and offer cryptographic proofs. This architecture fosters a decentralized data services layer,
where independent operators can deliver value-added services such as analytics, aggregation, and real-time data
pipelines—further decentralizing the network and enhancing data availability, security, and usability.

Block Nodes are free to innovate on payment for the provision of data (blocks, state etc) &mdash; they can be commercial and
have connectivity rules / rate limits / SLAs for customers with settlement in native cryptocurrency, web2 pay walls, or
at no cost.

Notably, in addition to replacing the cloud storage buckets, Block Nodes will:
- Take over block and state storage responsibilities.
- Provide reconnection services to consensus nodes and other Block Nodes.
- Offer cryptographic proofs to users and mirror nodes, enhancing security and decentralization.

![Block Node Architecture](../assets/hip-1081/images/block_node_network_architecture.svg)

The concepts of Block Node Discoverability and API pricing are introduced in this HIP but will be expanded upon in
future HIPs and individual Hiero deployed network documentation.

## Motivation

The current Hiero network architecture depends heavily on centralized cloud infrastructure for storing consensus node
output, including events, records, sidecars, state snapshots, and signatures. While this approach has supported the
needs of mirror nodes and indexers since 2019, it has introduced several limitations that conflict with the goals of
decentralization, efficiency, and extensibility.

The decision to introduce Block Nodes stems from three primary shortcomings in the existing architecture:

1. **High cost of access and data extraction** – Cloud-based storage solutions make accessing, parsing, and
distributing consensus data to mirror nodes expensive, particularly at scale.

2. **Centralized trust model** – Storing critical network data within a small number of centralized cloud
providers undermines the network’s decentralization and resilience.


3. **Limited state accessibility** – Developers and users currently lack APIs for querying live or historical state
data, making it difficult to verify or consume network state in a trustless manner.

Block Nodes are designed to decentralize data storage and access, enabling independent operators to serve verified
block and state data directly from the network. This allows users and applications to consume the data without
relying on enterprise-operated infrastructure or cloud-specific implementations.

- Additional motivations include:
Eliminating the current dependency where consensus nodes must connect to each other to sync Hashgraph history.

- Enabling the delivery of cryptographic proofs, including state proofs and block item proofs, which consensus nodes do
not provide today.

- Supporting a more open and inclusive node ecosystem where community members can run full-featured nodes with access
to the complete network history and state for verification or downstream services.

Block Nodes provide a clean architectural separation of responsibilities: consensus nodes focus on consensus, while
Block Nodes focus on data availability, accessibility, and verifiability—paving the way for a more decentralized and
...
centralized infrastructure, improve access to network data, and empower a wider range of developers and operators to
...
architectural consistency across the network. It also differentiates Block Nodes from typical Web3 node designs, which
often rely on JSON-RPC over HTTP, a less efficient and less structured approach for complex data streams.
...
software to maintain network integrity—Block Nodes are intentionally decoupled from the consensus layer. This enables
independent deployment and upgrade cycles without requiring coordination with the core network.
This independence increases the resilience and diversity of the network by allowing third-party developers and
...
fields, it should still be able to parse, verify, store, and serve block data reliably. This ensures network continuity
and user access to verifiable data, even during version transitions.
...
- **Independent Community Node Operators**: Community or commercial entities running Block Nodes to support
    decentralized data access, historical state services, or value-added APIs.
...
- **Auditors / Analysts / Researchers**: Users performing independent validation of transactions, querying historical
    state, or analyzing network behavior for compliance or insight
...
1. As a Block Node Operator, I want to ingest block streams from one or more Consensus Nodes, so that I can build a
complete and reliable record of network activity.
2. As a Block Node Operator, I want to verify block proofs, so that I can ensure the authenticity and integrity of the
data I store and serve.
...
4. As a Block Node Operator, I want to serve real-time block streams via a `subscribe` API, so that clients can access
up-to-date network activity as it happens.
...
parties can verify the correctness of network data without relying on trust in my services.
8. As a Block Node Operator, I want to support Reconnect APIs for syncing new or lagging nodes, so that I can help
other nodes rejoin the network efficiently and securely.
...
10. As a Block Node Operator, I want to expose a publish endpoint for upstream producers, so that Consensus Nodes or
other Block Nodes can push block items directly to me.
11. As a Block Node Operator, I want to expose a serverStatus API to report my current capabilities, so that clients
...
2. As a dApp Developer, I want to query historical state, so that I can understand application behavior over time and
debug or audit contract interactions.
3. As a dApp Developer, I want to fetch inclusion or state proofs for validation in a low-trust model, so that I can
build applications that do not require trust in centralized infrastructure.
...
- **Network Services**  – Ingesting, verifying, and storing output data from Hiero consensus nodes.
- **Data Access Services** – Providing value-added services that make network data and insights easily accessible to
...
| **Archive Block Node**            | A block node that maintains block history but does not expose consumer block streaming services.  |
...
trusted node operators. For optimal performance, Tier 1 nodes are expected to be deployed in close proximity to
...
Block Nodes will support the following core network functions:
- **Block Stream Ingestion** – Receiving and processing block streams from consensus nodes or upstream Block Nodes.
- **State Management** – Verifying and maintaining network state, and optionally persisting state backups.
- **Reconnect** – Providing bootstrapping and synchronization services to other nodes rejoining the network.

#### Block Stream Ingestion

Hiero is a modular blockchain network in which consensus nodes use the Hashgraph algorithm to order and timestamp
transactions. These nodes emit block streams containing all relevant transaction, state, and metadata outputs. For the
network to remain operational and responsive, this output must be efficiently ingested and processed by downstream
infrastructure, namely Block Nodes. Block Nodes are therefore a critical component in ensuring that consensus output
is reliably processed, stored, and made available to users and downstream services.
...
| `RESET`           | The Publisher reached a reset point. |
| `TIMEOUT`         | The delay between items was too long. |
| `ERROR`           | The publisher encountered an error. |
| `TOO_FAR_BEHIND`  | The block node is too far behind to catch up directly. |
...
providing cryptographic assurance that the block was verified by the network. If a block proof is absent for an
...
In some cases, a delayed block proof may arrive indirectly through a cascade proof, where a later block includes a
signature that retroactively validates a chain of prior blocks via embedded hash links. Block Nodes must be capable
of recognizing and validating such proofs to maintain a complete and verifiable block history.


#### State Management

In today’s network, consensus nodes periodically take state snapshots and store them in centralized cloud buckets.
These backups are primarily used for node bootstrapping, disaster recovery, and long-term archival. However, this data
has not been made readily accessible to the broader community, limiting its utility for external developers and
applications.
By maintaining live state and capturing periodic state snapshots, Block Nodes can introduce new state-related services
that provide DApps and external tools with granular access to network state at any block height. These capabilities
unlock use cases such as historical state queries, lightweight state verification, and state diffing over time.
Additionally, Block Node state snapshots can be used by other consensus or Block Nodes to efficiently bootstrap or
synchronize with the Hiero network, reducing recovery time and improving resilience.
...
| ------------------------- | ------------- |
| `UNKNOWN`                 | The server software failed to set a status,and SHALL be considered a software defect. |
| `INSUFFICIENT_BALANCE`    | The requesting client account lacks sufficient HBAR to pay the service fee for this request. |
...
It is common for a consensus node—or a Block Node—to fall behind due to network latency, load, or temporary outages.
Additionally, new nodes joining the network must be bootstrapped with the latest state and Hashgraph information to
synchronize and begin participating in consensus or data services.
...
To support network scalability and improve role specialization, Block Nodes will assume this teacher function. They
will act as reliable sources for consensus or Block Nodes that need to bootstrap or resynchronize with the network,
reducing load on consensus infrastructure and improving overall recovery and onboarding performance.
...
The Data Access role defines how Block Nodes transform raw network output into meaningful, consumable formats via APIs,
cryptographic proofs, and streaming protocols. This functionality forms a vital bridge between the Hiero protocol and
its surrounding ecosystem, enabling use cases such as state lookups, historical data queries, filtered block streams,
and trust-minimized validation of transactions. By decoupling data access from the consensus layer, Block Nodes
increase network scalability while fostering a more diverse and decentralized application landscape.
...
encouraged to leverage, extend, and customize to support a wide range of applications and network needs.
...
| `NOT_FOUND`       | The requested block was not found. |
| `NOT_AVAILABLE`   | The requested block is not available on this block node server. |
...
network activity as it occurs—without maintaining consensus node infrastructure.
...
2. The Block Node validates the request:
    ...
    - If the requested end block exceeds available data, it responds with `INVALID_END_BLOCK_NUMBER`.
    - If the end block is set to 0, the stream is treated as open-ended, continuing until:
...
| `INVALID_END_BLOCK_NUMBER`    | The requested end block number is not valid. |
...
```
> In the future a Block Node code expose which value-added services are enabled (e.g., filtering, proofs, streaming modes)
...
correctness of network data—both in real time and historically.

As defined in HIP-1056, the block stream exposes three types of cryptographic proofs. 
...
To support authentication and monetization, each Block Node must possess at least one account on the network. This
...
successful delivery and optionally implement backpressure or rate control.
