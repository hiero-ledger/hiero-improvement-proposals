...
One of the primary motivations is the decentralization of netowrk data storage, thus reducing the centralization of
information in a few cloud providers.

Additonally, some notable challenges of the existing architecture that the block nodes hopes to resolve are:
...
performance streaming, use of protobufs (continued commitment to simple and clean API specification) and
...
consensus nodes.
1. As a block node operator I want to accept blocks from one or more consensus nodes.
1. As a block node operator I want to be able to tell any consensus node to skip the current block and resume sending data
with the next block so that a consensus node does not waste resources sending a block a block node has already recieved.
...
4. As a block node operator I want to offer a streaming API that provides access to real time unverified block stream
details.
5. As a block node operator I want to offer state APIs that provide access to historical state details.
6. As a block node operator I want to offer state APIs that provide access to latest state details.
7. As a block node operator I want to answer all queries available from the consensus node.
8. As a block node operator I want to answer additional queries not available from the consensus node.
9. As a block node operator I want to “catch up” with the latest block(s) from a consensus node if my node falls
behind the latest block.
10. As a block node operator I want to “catch up” with the latest blocks from other block nodes if my node falls too
far behind the latest block.
11. As a block node operator I want to receive the unfiltered and unverified block stream from one or more block nodes,
from one or more consensus nodes, or from both.
12. As a mirror node operator I want to receive the unfiltered and unverified block stream from a block node with the
minimum possible latency.
13. As a custom dApp I want to receive an unfiltered and unverified block stream from a block node with the minimum
possible latency.
14. As a custom dApp I want to receive the block stream starting at any historical block and continue forward from that
point indefinitely.
15. As a custom dApp I want an API to request a copy of the periodic historical state snapshots maintained and made
available by a block node.
16. As a custom dApp I want an API to request a state proof for any item in state, as of an arbitrary historical block,
from a block node. Provided a snapshot is available for that block, or I can pay the cost to recreate state as of that
block.
17. As a custom dApp I want an API to request a state proof for any item in state, as of the most recent block, from a
block node.
18. As a consensus node operator I want to publish a block stream to one or more block nodes so that I am not required
to maintain historical state snapshots.
19. As a consensus node operator I want to retrieve a very recent state snapshot from a block node if I “fall behind”
and must “reconnect” with the network.
20. As a consensus node operator I want to have an “address book” of block nodes where I can send my block stream
and/or request a state snapshot or recent blocks.
21. As a consensus node operator I want a configurable trust score for block nodes listed in my “address book” so I can
...
3. No Block Proof: Each block stream in the block stream (with the exception of record file
    ...
    maintain its connection and will manage an end-of-stream in the case the CN falls behind. Here, open subscriber
    connections that are streaming live block items out will likely be terminated after an established period of time.
...
4. Delayed Block Proof: A delayed block proof may be followed by the receipt of a cascade proof. This is a where an
...
service BlockStreamService {
    /**
     * As a block stream producer publish a stream of blocks to a block node.
     */
...
#### subscribeBlockStream: Pull mode
In addition to ingesting blocks the block node will also support the export of the block stream data in a similar
manner to the consensus node. 

The block node will serve unverified block streams for low latency applications. In this case consuming clients are
encoruaged to verify the blocks utilizing the Bloock Proof algorithm.
...
The `getBlock` endpoint will allow a consumer to request a single verified block from the block node.
...
service ProofService {
   ...
   */
  rpc blockContents(BlockContentsProofRequest) returns (BlockContentsProofResponse);

  /**
   * Request a state proof for a given state item and block from the block node.
   */
  rpc state(StateProofRequest) returns (StateProofResponse);
}
```
...
To achieve this it is required that a block node have it's own AccountId that users can transfer to.
