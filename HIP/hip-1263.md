...
enum ResponseCodeEnum {
     ...
     */
    STALE = 514;  // Next available code in sequence
}
```
...
When a transaction with a STALE status is resubmitted to the network, it transitions back to UNKNOWN status on ingest,
or when first encountered by pre-handle.
...
This HIP does not authorize automatic resubmission by consensus nodes, it only considers how this status code would be
forwards compatible with such future behavior.

### Impact on Mirror Node

Mirror nodes will not be aware of this status code directly. Stale events are included in block streams, but there are
no transaction records or receipts in the block stream for stale transactions.
...
2. **Test Cases**:
   ...
   - Verification that stale records / receipts are not included in block streams or record streams
...
- `TRANSACTION_IN_STALE_SELF_EVENT`: We want to support reporting for all transactions not just those in self events
