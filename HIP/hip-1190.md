...
`signatureMap`: a base64 encoding of a list of protobuf [`SignatureMap`](https://github.com/hashgraph/hedera-protobufs/blob/main/services/basic_types.proto) messages, one for each transaction in the `TransactionList` sent for signing. Each `SignatureMap` contains the signatures for the corresponding transaction.

#### Example

The application having created and submitted a `transaction` object, and got `signatureMap` as a response can add the new signatures to the `transaction` as follows:
...
```javascript
    ...
    const signatureMaps = result.signatureMap
    // Add signatures from each signature map to the corresponding transaction
