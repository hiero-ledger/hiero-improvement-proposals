...
EVM smart contract. It achieves this by combining each signature with a SNARK proof proving that the 
aggregate BLS public key (used to verify that particular aggregate signature) is a valid 
aggregation of BLS keys in the network address book for nodes with a threshold amount of weight.
...
argument of knowledge (SNARK) based on honest nodes contributing signatures to address book
...
inputs to the SNARK prover; compute the proof; and then gossip their vote for that proof as the 
...
reach consensus on the SNARK proof for a particular construction. The message in the 
`target_proof` field contains the SNARK proof itself in its `HistoryProof#proof` field; 
