...
Throttles in Hiero are used to limit the rate of certain operations, such as transaction processing. Most throttles
are applied at the frontend (ingest) layer, where they control how many transactions can be submitted to the network.
If a transaction exceeds the throttle limit, it is rejected before being sent to the consensus layer, at no cost to
the user.

![High Level Consensus Node Throttling](../assets/hip-1249/high-level-consensus-node.png).

After a transaction passes the frontend throttle, it is sent to the consensus layer, where it is processed by the
hashgraph consensus algorithm. It is gossiped among all the consensus nodes, ordered by the hashgraph, and then
executed. We then apply a backend throttle.

The backend throttle exists for two reasons:
1. To detect any malicious or dishonest nodes who submit more transactions than they are entitled to.
2. To throttle transactions, such as EthereumTransactions, which could not be throttled at the frontend.
