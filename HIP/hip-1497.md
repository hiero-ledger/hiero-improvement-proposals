...
created: 2026-05-03
updated: 2026-06-18
requires: HIP-1494, HIP-1495, HIP-1496
release: TBD
---

# HIP-1497 - Consensus Node Migration to Kubernetes-Native Deployment
...
in HIP-1495. Migration is a per-node, independently-timed operation that keeps the network
...
Provisioner model defined in HIP-1495. This HIP addresses the one-time migration path for each of the 39 mainnet
...
HIP-1495 introduces a Kubernetes-native deployment model for Hedera consensus nodes. In the
...
Controller (UC) sidecar. The full architecture is described in HIP-1495.
...
modify the marker file protocol (HIP-1496), or the upgrade process itself.
...
- Describing the Kubernetes-native deployment architecture (covered in HIP-1495)
...
| **Marker file protocol preserved** | The UC sidecar consumes the same marker files as the legacy UC (HIP-1496). No CN code changes are required.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
`solo-provisioner`) that handles both network upgrade coordination (HIP-1496) and migration soak
...
by HIP-1496, which locks in the marker file protocol as the interface between the CN and its
...
Migration observability follows the same two-channel model used by the upgrade protocol
(HIP-1496) for daemon and CLI components: a crash-safe JSONL events file on disk, and
structured logs exported via OpenTelemetry to operator-chosen backends.

All JSONL event entries use the same schema as the upgrade protocol (HIP-1496 § Observability):
...
The daemon uses the same two-channel observability model as for network upgrades (HIP-1496):
...
Kubernetes Events are not emitted by the daemon. The `solo-provisioner-daemon` is a
systemd service running outside the cluster; K8s events are designed for controller-to-controller
communication within the Kubernetes ecosystem and are not the right tool for host-level process
reporting. JSONL + OpenTelemetry logs provide equivalent coverage without the RBAC overhead or
ephemeral TTL limitation.
...
Migration follows four phases. Before starting, ensure you have completed the HIP-1495 deployment prerequisites (CRI-O
