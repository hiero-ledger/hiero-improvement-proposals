...
status: Active
last-call-date-time: 2026-02-24T07:00:00Z
...
- Users to choose based on their security and convenience preferences. Note: RSA is suggested for the majority of the users. ECIES requires direct access to the private key to decrypt messages, so it's less "Wallet-friendly".
...
- `signature`: Hex-encoded signature over the Hedera-prefixed canonical JSON of the payload (see Signature Generation below)

**Signature Generation:**

1. Serialize the `payload` object using canonical JSON (keys sorted alphabetically, no whitespace)
2. Prepend the Hedera message prefix to produce the signed string:

   ```text
   "\x19Hedera Signed Message:\n" + canonicalJSON(payload).length + canonicalJSON(payload)
   ```

   where `.length` is the byte length of the UTF-8 encoding of the canonical JSON string.

3. Sign the prefixed string (encoded as UTF-8 bytes) with the message box owner's Hiero private key
4. Encode the signature in hexadecimal format
5. Include the signature and signer's public key in the `proof` section

This prefix matches the `hedera_signMessage` convention, ensuring that signatures produced by wallets and software/hardware signers are identical and interchangeable.
...
5. Prepend the Hedera message prefix:

   ```text
   "\x19Hedera Signed Message:\n" + canonicalJSON(payload).length + canonicalJSON(payload)
   ```

6. Verify the signature using the prefixed string (as UTF-8 bytes) and the account's public key
7. If verification fails: warn user and refuse to send message
...
For cryptographic signature verification to work reliably, implementations MUST use canonical JSON serialization combined with the Hedera message prefix when signing and verifying the first message payload. This is critical because:
...
- The Hedera prefix prevents accidental signing of arbitrary Hiero transactions while ensuring interoperability with wallet-based signers (`hedera_signMessage`)
...
The bytes actually signed are the UTF-8 encoding of:
...
Implementations in other languages MUST follow the same canonicalization and prefixing rules to ensure interoperability.
...
1. **Message Box Library** (`hiero-message-box-lib/src/message-box.js`):
...
2. **Encryption Library** (`hiero-message-box-lib/src/crypto.js`):
...
3. **Message Format Library** (`hiero-message-box-lib/src/format-message.js`):
   - Custom CBOR encoder/decoder (RFC 8949 compliant)
   - Automatic format detection (JSON/CBOR/plaintext)
   - Message display formatting and decryption routing

4. **Hedera Integration** (`hiero-message-box-lib/src/hedera.js`):
...
5. **Command-Line Tools** (`hiero-message-box-cli/src/commands/`):
   ...
   - `link-message-box.js`: Re-attach an existing topic to the account memo
...
- **SDK**: `@hiero-ledger/sdk` v2.80.0 or higher
- **Runtime**: Node.js v18+
- **Crypto**: WebCrypto (`globalThis.crypto.subtle`) for RSA operations (isomorphic — works in Node.js 18+ and browsers); Node.js `crypto` module for ECIES and ECDSA_SECP256K1 (Node.js only)
...
- Minimal external dependencies (only `@hiero-ledger/sdk`)
