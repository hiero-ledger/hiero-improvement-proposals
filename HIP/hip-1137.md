...
1. As an Hiero registered node operator I want to associate an Hiero account
   with my node.
...
Add three HieroFunctionalities, three schedulable transactions,
...
- `/api/v1/network/registered-nodes`
  ...
  - `order` - The direction to sort the items by the registered node id in the
    response. Can be `asc` or `desc`, default is `asc`
  - `registerednode.id` - The registered node id. Supports `eq`, `gt`, `gte`,
    `lt`, and `lte` operators. Only one occurrence of each operator is allowed.
    The `eq` operator can't mix with other operators
  - `type` - The type of service provided by the registered node. Supports
    `BLOCK_NODE`, `GENERAL_SERVICE`, `MIRROR_NODE`, and `RPC_RELAY`. Only
    supports the `eq` operator and only one occurrence is allowed

- `/api/v1/network/nodes`

  Add `associated_registered_nodes` to the response. It's the list of the ids
  of the registered nodes associated with a consensus node.
...
Each `RegisteredNode` may list many endpoints that offer a number of different
APIs. Each `RegisteredNode` represents a single logical node, however, and all
of the API endpoints for that node should, regardless of actual implementation,
behave as though provided by a single logical system.
...
1. Use of the existing consensus node address book and transactions was
   ...
   * This was rejected for three reasons.
      1. The existing Node transactions and state objects include a substantial
         amount of data that has no relevancy to other registered nodes
      1. The consensus nodes do not, and should not, include the additional
         information specific to some registered nodes
