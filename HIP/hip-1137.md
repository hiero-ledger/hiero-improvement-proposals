...
<dd>A node that participates in an Hiero Ledger network, but is not a
...
network to govern the network and ensure that network-wide decisions are
proper and correctly effected. One example is the Hedera governing council.</dd>
</dl>
...
1. As a Mirror Node, I want to present discoverable nodes to clients using
   on-chain data.
1. As an Hiero community member, I want to operate a discoverable node and
...
1. As a discoverable node operator, I want to publish and manage my discoverable
   node on an Hiero ledger so that clients can reliably find and connect to my
   service.
1. As an Hiero network governing entity I want the option to reward trusted
...
1. As an Hiero network governing entity I want the option to remove obsolete or
   misbehaving discoverable nodes.
1. As an Hiero network governing entity I want to ensure that creation of
   discoverable nodes occurs at a reasonable rate and the fees fully compensate
   the network for the long-term cost of maintaining the list of discoverable
   nodes.
...
message DiscoverableNodeCreateTransactionBody {
     ...
     * If a node rewards account ID does not resolve to an existing and active
     * account that is capable of receiving the rewards when discoverable
     * node rewards are calculated, that node SHALL forfeit that specific
     * reward payment (if any).<br/>
     * Forfeit of any one node reward payment SHALL NOT impact previous or
     * future node reward payments.
     */
...
structure to administratively delete a discoverable node when necessary.
...
message DiscoverableNodeDeleteTransactionBody {
     ...
     * This value is REQUIRED.
     * <p>
     * A given value for `discoverable_node_id` SHALL be unique within a given
     * shard or realm.<br/>
     * A given value for `discoverable_node_id` SHALL NOT be reused, even if the
     * corresponding entry is deleted.
     */
    uint64 discoverable_node_id = 1;
}
```
...
message DiscoverableAddressBookNode {
     ...
     * <p>
     * A given value for `discoverable_node_id` MUST be unique within a given
     * shard or realm.<br/>
     * A given value for `discoverable_node_id` MUST NOT be reused, even if the
...
The following are changes to the existing API.
...
Added `discoverable_node_id` in `TransactionReceipt`:
...
message TransactionReceipt {
    ...
    /**
     * The identifier of a newly created DiscoverableNode.
     * <p>
    ...
    */
    uint64 discoverable_node_id = 16;
...
same `discoverable_node_id` field in the transaction receipt.
...
1. Use of the existing consensus node address book and transactions was
   ...
   * This was rejected for three reasons.
      ...
      1. The modification of discoverable nodes and consensus nodes have
...
## Open Issues
1. Should we use a full Entity ID (0.0.1234 form) for discoverable node ID
   values, or is it reasonable to keep just a single integer (e.g. 28 or 18273)
   as the discoverable node ID.
