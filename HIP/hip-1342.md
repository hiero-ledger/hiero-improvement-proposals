...
discussions-to: https://github.com/hiero-ledger/hiero-improvement-proposals/pull/1397
...
async function main() {
  ...
  const canonicalCalldata = iface.encodeFunctionData("transfer", [decoded[0], decoded[1]]);
  const trailingData = "0x" + input.slice(canonicalCalldata.length);

  console.log("Calldata:", input);
  console.log("Decoded transfer:", decoded[1].toString(), "units to", decoded[0]);
  console.log("Trailing data:", trailingData);
}
...
// Trailing data: 0x3372348f476a87cf942167a35e78e40f00c91c99072b0c515ba8118cbd927fb7
```
