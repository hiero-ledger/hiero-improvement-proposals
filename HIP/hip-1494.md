...
The new package format defined in this HIP reduces the upgrade artifact to a small manifest — a few kilobytes of
machine-readable YAML files in a `manifests/` directory. Software is delivered as pre-built OCI images pulled from
council-hosted registries rather than reconstructed from JARs on each machine. This eliminates both the HFS upload
bottleneck and the on-machine image build step from the upgrade cycle. See **HIP-1495** for how the deployment model
