...
The current Hedera upgrade package is a ~100 MB zip archive containing the consensus node JARs, library dependencies,
and configuration scripts. It is uploaded to the Hedera File System (HFS) via thousands of small transactions before
each network upgrade — a process that is slow, fragile, and accounts for a significant portion of the total upgrade
cycle time. Each node then builds its own Docker images from these JARs during the upgrade window, introducing
per-node variability and further extending the time nodes spend offline.

The new model requires that container images be built deterministically from source — once, reproducibly, as part
of the software release process — and published to council-hosted OCI registries before the upgrade package is
distributed. Because the image is already built and its digest is known at release time, the upgrade package no
longer needs to carry JARs. It carries only a small manifest — a few kilobytes of machine-readable YAML in a
`manifests/` directory — that references the images by digest. Nodes pull from the registry instead of building
locally, eliminating both the HFS upload bottleneck and the on-machine image build step from the upgrade cycle.
See **HIP-1495** for how the deployment model consumes these manifests.
...
legacy UC daemon) and `manifests/` + `data/config/` content (consumed by the K8s-native UC sidecar), and
each node must apply only its relevant portion. See HIP-1497 for the migration protocol and the retirement
trigger for JAR-based artifact production.

### Package Specification

The deployment package serves multiple lifecycle operations: council-driven network upgrades, fresh node
provisioning, and network bootstrapping. Defining a single authoritative specification gives node operators,
tooling authors, and council members a stable contract to build against.
...
`external-files.yaml`. The `phase` structure separates the download timing (`prepare` — before the freeze window) from the install timing (`freeze` — after `now_frozen.mf`, when the CN is stopped), giving tooling precise control over when network and I/O load occurs.
...
- **Required entries** (default, `optional` absent or `false`): any failure — insufficient disk space,
  ...
  installed, and the downloader emits a `FileDownloadFailed` or `FileHashMismatch` JSONL entry and logs the
  error with the entry URL and reason. This is the safe default: a declared file that cannot be delivered leaves the host in a
...
The `url` field must use one of the following schemes:

| Scheme | Transport |
|--------|-----------|
| `https://` | HTTPS download via standard HTTP client |
| `s3://` | AWS S3 (or S3-compatible) object storage |
| `gcs://` | Google Cloud Storage |

The scheme determines which download client the daemon selects. URL schemes not in this list are rejected
at parse time with a `ManifestInvalid` error. This mirrors the scheme enforcement already applied to
`state-sources.yaml` bucket URIs. The `https://` scheme is accepted here even though `state-sources.yaml`
restricts to native cloud schemes, because individual large files are often published at plain HTTPS URLs
(e.g., release artifacts hosted on a build server).
...
files:
  - url: "s3://hedera-artifacts/some-key.bin"
    ...
    phase:
      ...
      install: freeze      # Stage to destination during the freeze window (CN stopped)
...
- `freeze` — staged to the final destination during the freeze window, after the CN has stopped (`now_frozen.mf` received).

#### `archives:` — Verifiable Archive Download and Extraction

Some large, versioned payloads are published as a single archive containing multiple members rather than as
individually addressable files. Declaring each member as a separate `files[]` entry either requires the
artifact publisher to host members individually (discarding the archive's provenance), or forces an
out-of-band unpack step that operates outside the manifest's integrity contract. The `archives:` key handles
this natively.

`archives:` is a top-level key, sibling to `files:`, not a sub-block on `files[]` entries. The distinction
is intentional: a `files[]` destination is a file path; an `archives[]` destination is a directory. Merging
the two shapes into one entry forces every consumer to branch on entry shape before it can validate anything.
The verification contract and failure modes are also genuinely different (see below).

**Concrete motivating case:** TSS WRAPS recursive-proof proving keys. `tss.wrapsEnabled=true` requires four
artifacts (`decider_pp.bin`, `decider_vp.bin`, `nova_pp.bin`, `nova_vp.bin`) staged before the node starts.
These are published as a single `wraps-v1.0.0.tar.gz` (~2 GB). Re-hosting the members individually or relying
on out-of-band extraction are both worse than a first-class manifest entry.
...
archives:
  - url: "https://builds.hedera.com/tss/hiero/wraps/v1.0/wraps-v1.0.0.tar.gz"
    format: "tar.gz"             # tar.gz | tar | zip — explicit, never inferred from the URL
    algorithm: "sha256"          # Hash algorithm for the archive as downloaded
    checksum: "abc123..."        # Expected hash of the archive as downloaded
    contentType: "application/gzip"   # Optional
    destination: "HAPIAPP_DIR/data/keys/wraps"   # A DIRECTORY — resolved via the same marker-prefix set as files[]
    optional: false              # Default; failure aborts the operation
    phase:
      download: prepare          # Same vocabulary as files[].phase.download
      install: freeze            # Same vocabulary as files[].phase.install
    contents:                    # REQUIRED and AUTHORITATIVE — see invariant below
      - path: "decider_pp.bin"   # Member path within the archive (relative, no leading /, no ..)
        algorithm: "sha256"
        checksum: "def456..."
        size: 2170538264         # Optional; used for disk-space pre-check without trial extraction
      - path: "decider_vp.bin"
        algorithm: "sha256"
        checksum: "789ghi..."
      - path: "nova_pp.bin"
...
An optional per-member `destination` override (subject to the same marker-prefix policy) covers flattening
or relocating members out of a nested archive structure. When absent, the install path for a member is
`<archive destination>/<member path>`.

**Authoritative-contents invariant (hard rule):**

`contents` is the complete and authoritative description of the archive. Set equality is enforced in both
directions:

- A regular-file member present in the archive but **absent from `contents`** → **abort**. The archive is
  not what the manifest declares; treating extraneous content as benign is how an extra payload rides along
  inside a signed package.
- A `contents[].path` entry **absent from the archive** → **abort**.

Directory entries in the archive are ignored for the equality check (they are structural). Any non-regular
member — symlink, hardlink, device node, FIFO, socket — is rejected outright and never extracted, closing
the tar-slip / zip-slip class of path-escape vulnerability.
...
- `format` must be one of the closed set `tar.gz | tar | zip`. It is never inferred from the URL extension — URL extensions are attacker-influenced metadata.
- `destination` must begin with a recognised marker prefix followed by `/`, using the same closed set as `files[]`.
...
- Every resolved install path — across all `archives[]` members and all `files[]` entries — must be globally unique. Two entries silently overwriting each other at install time is the same defect the existing `files[].destination` uniqueness rule guards against; the archive case widens the namespace that must be checked.

**Processing rules (apply time):**

1. Verify sufficient disk space for the archive download size and the total uncompressed size before fetching anything. The optional `size` field on the archive entry and on each `contents[]` entry makes this computable ahead of time, without a trial extraction.
2. Download the archive. Verify the archive checksum before extracting anything.
3. Extract to a staging directory on the same filesystem as the final destination — never directly into the destination.
4. Enumerate members and enforce the authoritative-contents invariant before any per-member checksum work.
5. Verify every member's checksum in staging.
6. Install atomically as a unit: move members into place only after every member has been verified. A partially installed archive leaves the host in exactly the partial state the required-entry rationale warns against.
7. `optional: true` applies to the archive as a whole. There is no partial-member success.

**Disk-space pre-check:** covers the archive download size plus the total uncompressed size. The optional
`size` field on the archive entry and on each `contents[]` entry enables this check without a trial
extraction or trusting the archive's own embedded headers.

**Error classifications:**

| JSONL entry | Condition |
|---|---|
| `ArchiveDownloadFailed` | Transport, HTTP, or disk-space failure fetching the archive |
| `ArchiveHashMismatch` | Archive checksum does not match `checksum` |
| `ArchiveContentsMismatch` | Archive membership is not exactly `contents` (either direction) |
| `ArchiveMemberHashMismatch` | An extracted member's checksum does not match its `contents[]` entry |
| `ArchiveUnsafeMember` | Non-regular member, or a member path that escapes the destination |

These follow the same naming convention as the existing `FileDownloadFailed` / `FileHashMismatch` /
`ManifestInvalid` entries.
...
```yaml
operator.solo.hiero.org/operation-id: <operation identifier for the surrounding operation; for upgrade phases this is the upgrade's operationId, minted by the UC at the prepare phase (see HIP-1496)>
operator.solo.hiero.org/job-file: validate-config      # matches descriptor filename stem (no ordering implied by filename)
```
...
### Consensus Node Runtime Start/Stop Control

The deterministic consensus-node image runs the CN process under the
[s6-overlay](https://github.com/just-containers/s6-overlay) supervision suite (`ENTRYPOINT ["/init"]`), which
manages the JVM as an s6 long-run service named `consensus`. This decouples the *container* lifecycle from the
*CN process* lifecycle and defines a stable control interface that the deployment tooling (HIP-1495) relies on.
No additional package artifact is required — the contract below is a property of the image itself.

**Boot behaviour — `AUTO_START_CONSENSUS_SERVICE`.** The `consensus` service reads the
`AUTO_START_CONSENSUS_SERVICE` environment variable (default `true`) to decide whether to launch the CN when the
container starts:

| Value            | Behaviour on container start                                                                                                    |
|------------------|---------------------------------------------------------------------------------------------------------------------------------|
| `true` (default) | The CN process starts automatically. If it later exits it is **not** auto-restarted (the service is marked do-not-restart).      |
| `false`          | s6 comes up but the `consensus` service exits immediately without launching the JVM. The container is healthy and **idle**; the CN is started later, out-of-band. |

The service distinguishes a boot-time auto-start from an operator-initiated start (via `s6-svstat -o wantedup`),
so `false` suppresses only the automatic boot start — not a subsequent explicit start.

**Control interface.** Once the container is running, the CN process is controlled **in place — without
restarting the container** — using the s6 commands already present in the image:

| Action | Command                                                     |
|--------|-------------------------------------------------------------|
| Start  | `s6-svc -o /run/service/consensus`                          |
| Stop   | `s6-svc -d /run/service/consensus` (graceful SIGTERM)       |
| Query  | `s6-svstat -o up /run/service/consensus` → `true` / `false` |

Start uses `-o` ("up once") deliberately: it launches the CN without marking it permanently up, keeping it
distinct from a boot auto-start and avoiding an s6 restart loop. Because the service is marked do-not-restart, a
CN that exits (crash or explicit stop) stays down until it is explicitly started again; the container keeps
running throughout.

This interface is consumed by `solo-operator` (HIP-1495), which sets `AUTO_START_CONSENSUS_SERVICE` from a
deployment-policy field and drives start/stop through the commands above. It is a **stable contract**: the
environment-variable name, the service path (`/run/service/consensus`), and the command semantics must not change
without a coordinated update to the consuming HIPs.
...
|-------|-----------------------------------------------------------------------------------------------------------|
| 1495 | Dedicated config CR kinds per file; `data/config/` directory convention; `consensus-node-components.yaml`; CN runtime start/stop control contract |
...
- **Additive changes** (new optional fields, new optional top-level keys in a manifest file): permitted
  within the same `schemaVersion`. All tooling (operator, daemon, UC) **must** use lenient parsing —
  unknown fields and unknown top-level keys are silently ignored. This is what makes additive changes
  non-breaking: a newer package can be processed by an older component without failure, as long as the new
  content is optional and its absence does not change existing behaviour. A consumer that uses strict field
  decoding and rejects unknown keys violates this contract and turns every additive change into a breaking
  one in practice, regardless of `schemaVersion`.
- **Breaking changes** (removed fields, renamed fields, changed field semantics, new required fields):
  ...
  and emit a `ManifestInvalid` JSONL entry and log the error rather than silently misinterpreting the content.
...
configuration. `operationId` is minted by the UC at the start of the prepare phase (from the node identifier,
`execute_immediate.mf` creation time, and package version — see HIP-1496) rather than being council-authored
in the package — removing a manual step that was a potential source of error. The resulting ID is per-node
(e.g. `node0-upgrade-v0.75.0-20260413T120000Z`), ensuring sibling nodes never collide on upgrade operations.
...
- [HIP-1495 - Kubernetes-Native Consensus Node Deployment](https://hips.hedera.com/hip/#hip-1495)
- [HIP-1496 - Globally Synchronized Network Upgrade Process](https://hips.hedera.com/hip/#hip-1496)
- [HIP-1497 - Consensus Node Migration to Kubernetes-Native Deployment](https://hips.hedera.com/hip/#hip-1497)
