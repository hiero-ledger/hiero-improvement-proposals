...
HTS system contract currently fails when extra bytes are present in calldata, preventing ERC-20-style calls with a trailing memo from succeeding. This breaks compatibility for integrations relying on the memo pattern and requires workarounds that degrade developer experience. Enabling HTS system contract to accept and ignore trailing calldata resolves this incompatibility while preserving consensus integrity and transaction provenance.

## Rationale

- EVM compatibility: Typical Solidity contracts do not revert solely due to trailing calldata; the decoder consumes expected parameters and disregards the remainder. Bringing HTS behavior in line with this expectation reduces surprise and increases portability of EVM-based tooling.
- Minimal scope: The change is localized to ABI parameter decoding within HTS system contract (and possibly shared libraries), with no changes to the HTS semantics or state transitions.
- Data availability: The full calldata remains in the Ethereum transaction input; off-chain systems can continue to read memos via standard RPCs (e.g., `eth_getTransactionByHash`). HTS memo fields remain unrelated and are still only visible via native APIs.
- Operational safety: Transaction size limits and fee/size checks already constrain worst-case payloads.

## User stories

- As a cross-chain router, I want to append an order ID to ERC-20 `transfer` calls routed through HTS, so I can correlate off-chain orders with on-chain settlements.
...
- As a dApp developer, I want my integration that works on other EVM networks to work on Hiero without special-case logic when using HTS system contract.

## Specification

This section defines the required behavior changes for HTS system contract and, optionally, other system contracts.

- Scope
  - Required: HTS system contract, including paths invoked via `redirectForToken` (see [HIP-218](https://hips.hedera.com/hip/hip-218)) that emulate ERC-20 functions (`transfer`, `transferFrom`, `approve`, etc.).
  - Optional (recommended): Apply the same decoding rule to other system contracts to ensure consistent behavior across the precompile/system contract surface.
...
- Semantics
  - State transitions and events emitted by HTS system contract are unchanged.
  - The extra trailing bytes have no semantic effect and are not persisted in HTS-specific fields, but remain visible in the Ethereum transaction input data.
...
- Concept: "Memo via calldata" is an EVM pattern where applications append memo or metadata to calls. HTS will accept and ignore these bytes, while preserving them in the transaction input for off-chain reading.
- Guidance: Continue using standard Ethereum JSON-RPC to obtain input data. Do not expect the HTS `memo` field to reflect the appended calldata.
...
- Update the ABI decoding library used by HTS system contract (and shared system contract decoding utilities if present) to:
...
- Introduce new HTS methods with explicit `bytes memo` parameters: Heavier API surface change with less compatibility benefit; ignores off-chain tools already reading calldata.
- Limit to only `transfer`: Applying uniformly across HTS (and, ideally, all system contracts) is simpler, less surprising for developers and users, and fully EVM-compatible.

## Open Issues

- Determine whether to enable this behavior across all system contracts.
