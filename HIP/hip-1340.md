...
Ethereum's Pectra upgrade (activated May 7, 2025) introduced, among other things, the facility of EOA (Externally Owner Account) Code Delegation. It has originally been defined in EIP-7702. This HIP defines the adoption of this feature in Hedera, Hedera-specific modifications and extensions of it, and the related changes to HSCS's proxy/facade mechanism.

## Motivation

EOA Code Delgation is the core of Ethereum's Pectra upgrade and enables a wide range of use cases. Supporting it aligns with the goal of maintaining compatibility with Ethereum Mainnet.
...
EOA Code Delgation has originally been defined in EIP-7702. We propose to adopt it with minimum changes.

### High-Level Summary

The primary feature of EOA Code Delegation is that an EOA can be configured to point to an EVM code stored in another on-chain entity (i.e. a Smart Contract).
...
This section describes how Code Delegation data for a given EOA is stored and what are the means of updating it.

#### Storing The Code Delegation Data

We propose to adopt the following storage format, which has been originally described in EIP-7702. 

Delegation information will be represented in a form of a Delegation Indicator - a fixed prefix of `0xef0100` followed by 20-byte EVM address of the delegation target.

```
delegation_indicator = 0xef0100 || 20-byte address
```

The Delegation Indicator will be stored as the code of an EOA using the facilities that currently exist for storing the code (runtime bytecode) of contract accounts - i.e. the `ContractStateStore` interface. To be able to utilize the interface, EOA's AccountID will be directly converted to a corresponding ContractID.

The main revision is that the code of an EOA will become mutable. It can be either empty or contain a Delegation Indicator. No other code is allowed.
...
The delegation target address is an arbitrary 20-byte address. No additional validation is done at the time of storing the Delegation Indicator (e.g. whether it points to a valid contract, etc), the only exception being `0x0000000000000000000000000000000000000000` - any attempt to store it will result in clearing the Delegation Indicator instead (i.e. setting the bytecode to an empty byte array).
...
If set, the value of the field will be checked to ensure it's a valid 20-byte EVM address. It will then be prefixed with the Delegation Indicator prefix, and stored as the Delegation Indicator for the given account. No other validation will be performed at the time of setting the Delegation Indicator (e.g. no validation that the target address exists, isn't a precompile, etc).

Since Delegation Indicators are persisted as the code (runtime bytecode) of EOAs, in addition to these protobuf updates, functionality will be added in the `TokenService` to call the `ContractService` to persist data into `ContractStateStore`.

This will be accomplished via the creation of a new `ContractServiceApi` whose interface will be defined as follows:
...
public interface ContractServiceApi {
    /**
     * Sets the given address as a delegation target of the specified account.
     * The address will be prefixed with `0xef0100` and stored as runtime bytecode of the given account.
     * Setting the target address to `0x0000000000000000000000000000000000000000` results in clearing
     * the Delegation Indicator of the given account (i.e. setting its runtime bytecode to an empty array).
     *
     * @param accountID         id of the account to set the delegation for
     * @param delegationTarget  the delegation target address
     */
    void setAccountDelegationTarget(@NonNull AccountID accountID, @NonNull Address delegationTarget);
}
```
...
Each successfully processed authorization item will result in a child CryptoCreate or CryptoUpdate dispatch (depending on whether the affected EOA already exists or not), carrying the intended Delegation Indicator change. No child dispatch will be created for duplicate authorizations that were skipped.

In addition to the gas cost for each authorization list item (both valid and invalid) defined in EIP-7702 - 12500 or 25000 gas depending on whether the EOA alredy exists - Hedera will charge hollow account creation cost of TODO for any newly created accounts. The maximum number of created hollow acounts is limited by transaction gas limit as well as the ops duration throttle.
...
- CryptoCreate and CryptoUpdate transaction bodies will be extended with a new field
- state_changes will contain a `STATE_ID_BYTECODE` state change for each updated Delegation Indicator
...
1. Fetch the bytecode of the target account of the call
2. Recognize the Delegation Indicator by inspecting the prefix of the code for `0xef0100`
3. Resolve the contract account that is the target of the delegation and run its EVM code in the context of the original EOA, subject to the limitations described below
...
Aside from code execution, the call might still carry a value transfer. The only difference in that regard is that if the code execution reverts, than the associated value transfer should also revert.
...
- targetting an existing EOA, which itself contains code delegation results in a no-op code execution
...
In case a Delegation Indicator points to another delegation, creating a potential chain or loop of delegations, only the first one will be resolved and the code of the target address will be interpreted literally, even if it's also a Delegation Indicator.
...
State Changes:
- EOAs will store their Delegation Indicators as their bytecode (as per EIP-7702)
- EOA's bytecode can be non-empty and becomes mutable
- EOA's storage slots can be non-empty and become mutable

Transaction Format Changes:
- CryptoCreate and CryptoUpdate will be extended with an additional field
- EthereumTransaction will be extended with a new EVM transaction type 4
- Some outputs of the transaction previously reserved for contracts will also support EOAs (bytecode change, storage slots change, EVM logs)
...
- CryptoCreate / CryptoUpdate will be updated to handle setting/updating Delegation Indicators
...
- EOA will store mutable bytecode. Mirror Node will need to extract and process EOA code changes from "state changes"
...
All supported SDKs will need to support parsing, creating and submitting EVM type 4 transactions.
