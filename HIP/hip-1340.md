...
We propose to extend `CryptoCreateTransactionBody` and `CryptoUpdateTransactionBody` with an additional field holding raw Delegation Indicator that the user wishes to store on their account:
...
Both fields, if set, will be validated to ensure they match the expected Delegation Indicator format. No other validation will be performed at the time of setting the Delegation Indicator (e.g. no validation that the target address exists, etc).
