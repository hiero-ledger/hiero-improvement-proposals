...
discussions-to: https://github.com/hiero-ledger/hiero-improvement-proposals/pull/1300
type: Standards
category: Core
...
This HIP proposes adding a new system privilege to Hiero governance accounts `accounts.treasury` and `accounts.systemAdmin`, enabling an increased transaction size limit from the current 6KB to 130KB. This change addresses challenges faced by the growing Hedera council, whose governance transactions increasingly exceed the standard transaction size limit due to the increasing number of required signatures. The proposed 128KB limit is already supported by Hiero's existing jumbo Ethereum transaction feature, minimizing implementation complexity and potential system disruptions.

## Motivation

The network governance council is responsible for the governance of a Hiero network, including signing off on major network changes such as upgrades, topology changes, and updates to fees or throttles. These governance actions require signatures from council members, which are performed from the privileged accounts `accounts.treasury` and `accounts.systemAdmin`.

As a Hiero council grows its membership, the number of keys associated with these accounts also increases. This growth has already created an operational challenge for the Hedera network where signed transactions from these accounts are exceeding the standard transactional size limit of 6KB, potentially impeding critical governance functions.
...
1. It aligns with Hiero's existing jumbo Ethereum transaction feature, which has already been tested and implemented.
...
The proposed approach leverages existing network capabilities with minimal changes, providing a straightforward solution to enable effective network governance for large and well governed Hiero networks.
...
3. **Implementation Approach**:
    - Add a conditional check in the transaction preprocessing logic that identifies if the payer account of the transaction is one of the privileged accounts (`accounts.treasury` or `accounts.systemAdmin`)
...
3. Hiero already supports the jumbo Ethereum transaction feature with a 130KB size limit
...
1. **Limited Scope**: Only two specific accounts (`accounts.treasury` and `accounts.systemAdmin`) will receive this privilege, both of which are already highly privileged governance accounts with strict access controls
2. **Existing Capability**: Hiero already supports 130KB transactions through the jumbo Ethereum transaction feature, so no new attack vectors are being introduced
3. **Governance Protection**: This change actually enhances security by ensuring governance operations can proceed smoothly in large and well governed networks, maintaining the intended security model of requiring multiple signatures

## How to Teach This

This change most immediately affects Hedera council members and network operators:
...
1. **For Network Operators**:
    - Be aware that transactions from accounts `accounts.treasury` or `accounts.systemAdmin` may be larger than the standard limit
