...
| 0x25bd252f  | scheduleDeleteTransaction(address)                                                                                                                                   |
...
This scheduled transaction may be deleted with the `scheduleDeleteTransaction()`, which
only reverts if it fails to perform the deletion.

The `hasScheduleCapacity()` view function never reverts, but returns `true` iff the given
second still has capacity to schedule a contract call with the specified gas limit.
