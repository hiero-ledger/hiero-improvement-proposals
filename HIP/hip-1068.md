...
- **`GAS_FEES`**: The allowance funds EVM gas costs for smart contract operations.
...
3. As a **smart contract platform**, I want to sponsor gas fees for contract interactions so that my users only need to
   sign transactions, not fund them.
5. As a **sponsor**, I want to set a maximum HBAR, USD, or GAS amount per spender per scope so that I retain budgetary control over
   sponsored fees.
...
13. As an existing account on the network I want to be able to sponsor up to a certain HBAR limit the cost of future
...
```protobuf
    enum AllowanceScope {
        ...
        *
        * TRANSACTION_FEES and GAS_FEES are separate concepts:
        *   - TRANSACTION_FEES = the HAPI-level fee from the fee schedule
        ...
        *
        * When this scope is used, the AllowanceUnit SHOULD be set to GAS
        * to express the amount in gas units. Gas is settled in HBAR at
        * the prevailing gas-to-tinycent conversion rate at execution time.
        */
        GAS_FEES = 2;
...
```protobuf
    enum AllowanceUnit {
         ...
         * This is the default value when AllowanceUnit is not explicitly set in CRYPTO_TRANSFER scope,
         * preserving backwards compatibility for all existing allowances.
         */
        HBAR = 0;

        /**
         * The allowance amount is denominated in gas units.
         * Gas is settled in HBAR at the prevailing gas-to-tinybar conversion
         * rate at execution time.
         *
         * GAS is ONLY valid when AllowanceScope is GAS_FEES.
         * Using GAS with any other scope (CRYPTO_TRANSFER, TRANSACTION_FEES)
         * will cause the transaction to fail with ALLOWANCE_UNIT_SCOPE_MISMATCH.
         *
         * Note: GAS_FEES scope also accepts HBAR and USD; GAS is the
         * default when no unit is explicitly set for that scope.
         */
        GAS = 1;

        /**
         * The allowance amount is denominated in USD (expressed in USD cents).
...
```protobuf
    ...
    */
    message CryptoAllowance {
        ...
        *
        * When not explicitly set, the default unit is determined by scope:
        *   - CRYPTO_TRANSFER  → HBAR  (tinybars) ; USD is also valid
        *   - TRANSACTION_FEES → HBAR  (tinybars) ; USD is also valid
        *   - GAS_FEES         → GAS   (gas units); HBAR and USD are also valid
        *
...
```protobuf
     ...
     * HAPI fees (TRANSACTION_FEES), EVM gas (GAS_FEES), or any combination.
     */
...
When a unit is not explicitly set in a `CryptoApproveAllowance` request for a **new** allowance, the scope-based default applies:
- `CRYPTO_TRANSFER` → `HBAR`
- `TRANSACTION_FEES` → `HBAR`
- `GAS_FEES` → `GAS` (default; `HBAR` and `USD` are also valid for this scope)
...
For each `sponsor_claim` in the transaction, in canonical scope order: `CRYPTO_TRANSFER` → `TRANSACTION_FEES` → `GAS_FEES`:
...
3. Determine the HBAR cost for the requested scope.
   ...
   - For `GAS_FEES`: estimate the maximum gas charge in tinybars using the gas limit and prevailing `gasPrice`. Actual gas consumed is not yet known at this phase; the gas limit is used as the upper bound for validation.
4. Convert the fee amount to the allowance’s denomination:
   ...
   - `GAS`: convert using the prevailing `gasPrice` (tinycents-per-gas × exchange rate), then compare against the stored amount in gas units.
...
7. Charge the sponsor’s HBAR balance for the actual tinybar cost.
   - For `GAS_FEES`: the actual gas consumed (known after EVM execution) is used for the final charge, which may be less than the gas limit used during validation. The allowance is debited by the actual consumed amount, not the estimate.
...
```protobuf
    enum ResponseCodeEnum {
        ...
        */
        ...
        /**
        * The AllowanceUnit specified is not valid for the given AllowanceScope.
        *
        * Valid unit/scope combinations:
        *   CRYPTO_TRANSFER  → HBAR, USD         (GAS not permitted)
        *   TRANSACTION_FEES → HBAR, USD         (GAS not permitted)
        *   GAS_FEES         → HBAR, USD, GAS    (GAS is the default)
        *
        * This error is returned when unit=GAS is submitted with any scope
        * other than GAS_FEES (e.g., unit=GAS with scope=TRANSACTION_FEES).
        */
        ALLOWANCE_UNIT_SCOPE_MISMATCH = <next_code>;
    }
```
...
```solidity
    interface IHRC1068 {
        ...
        enum AllowanceUnit {
            ...
            GAS,    // amount denominated in gas units; use with GAS_FEES scope
...
A new query param `?scope=(CRYPTO_TRANSFER|GAS_FEES|TRANSACTION_FEES)` would be added to allow users to filter on the specific applicable scope type.

A new query param `?unit=(HBAR|GAS|USD)` would be added to allow users to filter by allowance denomination.
...
4. **No changes to CryptoDeleteAllowance.** NFT allowance deletion continues to work as defined in HIP-336. To remove
   an HBAR, USD, GAS, or fungible token allowance of any scope, submit a `CryptoApproveAllowance` with `amount = 0` for the
   desired scope.
...
| Scopes covered | Value movement only | `CRYPTO_TRANSFER`, `TRANSACTION_FEES`, `GAS_FEES`, or any combination |
...
- A contract that reads an allowance and assumes `HBAR` denomination will compute incorrect budgets if the stored unit is `USD` or `GAS`. Contracts should always read and validate the `unit` field alongside `amount` before making spending decisions.
...
- [ ] Q: Can hollow accounts be given an allowance 
