...
Each Hedera consensus node today runs on a dedicated bare-metal machine managed by Docker Compose. Two
problems dominate the upgrade cycle. First, the upgrade package is a ~100 MB zip archive containing the
consensus node JARs and dependencies, uploaded to the Hedera File System via thousands of small transactions
before each network upgrade — a slow, fragile process. Second, each node builds its own Docker images from
those JARs during the upgrade window itself, which is non-deterministic across the fleet and means upgrade
time is directly proportional to image build speed on each individual machine. Beyond upgrades, there is no
declarative source of truth for node state, no built-in crash recovery, and configuration drift across nodes
...
- **Faster, smaller upgrades**: The upgrade package shrinks from ~100 MB (JARs + scripts) to a few kilobytes
  (manifest only). OCI images are pre-fetched from council-hosted registries before the freeze window, so the
  upgrade reduces to a pod swap — eliminating the image build step and making CN restart time the dominant
  factor in the upgrade window.
...
| Core     | `ConsensusNodeStart`        | Namespace |
| Core     | `ConsensusNodeStop`         | Namespace |
...
The `ConsensusNodeStart` / `ConsensusNodeStop` operation CRs follow the same pattern.
...
- **Start policy** — `startPolicy` (`Auto` | `Manual`, default `Auto`) selects who starts the CN process. In
  `Auto`, the operator sets `AUTO_START_CONSENSUS_SERVICE=true` (see HIP-1494) and the container self-starts and
  self-heals on pod restart; `ConsensusNodeStart`/`ConsensusNodeStop` operations are rejected. In `Manual`, the
  operator sets `AUTO_START_CONSENSUS_SERVICE=false`, the node boots idle, and the CN is started and stopped in
  place by the node-operator. `Auto` suits dev/test and cluster-only profiles; `Manual` suits controlled (mainnet)
  operations.
...
When `startPolicy: Manual`, the node boots **idle** and the operator never starts the CN on its own. A
`ConsensusNodeStart` starts the process in place; a `ConsensusNodeStop` stops it. These are **one-shot,
fire-and-forget** operations: the operator does not persist a desired run-state and does not self-heal, so after
any pod reschedule (or a CN crash) the replacement boots idle and **stays idle until a fresh `ConsensusNodeStart`
is issued**. Automatic recovery is deliberately left to `startPolicy: Auto` (where the container self-starts and
self-heals); `Manual` trades that away for explicit, node-operator-driven control — a routine reschedule taking a node
offline until a node operator issues a fresh Start is expected, and is surfaced through the node's platform-status
metrics. The
capsule reports a `Stopped` lifecycle phase whenever the CN process is not running under `Manual` — distinct from
`Offline` (which scales the pod to zero and removes the container) and from `Failed`. The operator actuates
start/stop in place through the CN image's s6 control interface (HIP-1494), so neither restarts the pod.
...
#### ConsensusNodeStart / ConsensusNodeStop (namespace-scoped)

`ConsensusNodeStart` and `ConsensusNodeStop` are imperative, single-shot **operation CRs** that start or stop the
consensus process of a target `ConsensusCapsule` **in place, without restarting the pod**. Like the
`NetworkUpgrade*` CRs, they are created by external actors (the operating CLI or automation) and only reconciled
by the operator — never created by the operator itself. Each must carry:

- **Target** — a name reference to the `ConsensusCapsule` (node) the operation applies to, within the same
  namespace.

Unlike the `NetworkUpgrade*` CRs, a Start/Stop operation is standalone — it is not part of a linked multi-CR
operation window — so it carries **no operation ID**. Idempotency is intrinsic: actuation is a no-op when the
node is already in the requested state (starting a running node or stopping a stopped one), so a duplicate or
re-applied CR triggers no re-execution, and redundant CRs are swept by the TTL/GC policy below.

The operator processes an operation as follows:

- **Precondition** — the target capsule must have `startPolicy: Manual`. Against an `Auto`-mode capsule the
  operation is rejected with a terminal `Failed` status and a clear reason, because the container would re-start
  the CN on its next boot regardless, making the operation non-durable. This cross-object check is performed by
  the reconciler, not by an admission policy.
- **Effect** — the operator actuates the transition in place via the s6 control interface (HIP-1494): `s6-svc -o`
  to start, `s6-svc -d` to stop. The operation is one-shot; no desired run-state is persisted, so it does not
  cause the node to be re-started after a later pod reschedule (see `ConsensusCapsule`, `startPolicy: Manual`).
- **Completion** — the operation reaches `Succeeded` when the CN **process** state matches the request
  (`s6-svstat -o up` returns `true` for Start, `false` for Stop) — not when the node reaches `ACTIVE` platform
  status. Platform-status progression is observed separately on the `ConsensusCapsule`.

The spec of an operation CR is immutable after creation, and completed operation CRs are subject to a
time-to-live / garbage-collection policy.
...
a spec update is required to unblock. Reconcilers emit Kubernetes events on their respective CRs for both
cases to aid observability; UC and daemon emit structured JSONL entries instead.
...
- **Create** `pods/exec` in managed namespaces — to actuate CN start/stop in place by running the s6 control
  commands inside the `consensus-node` container; required only for `Manual`-mode capsules (`ConsensusNodeStart` /
  `ConsensusNodeStop`)
...
> `data/config/` that exceeds this limit, emit a `ConfigFileTooLarge` JSONL entry, and halt the upgrade
...
Solo-operator reconcilers emit Kubernetes events on their respective CRs (`NetworkUpgrade*`,
`ConsensusCapsule`) at every significant lifecycle transition (image validation, registry failover, upgrade
phase changes, pod readiness). UC and daemon emit the same milestones as structured JSONL entries and stdout
log lines collected by Loki — they do not emit K8s events.
...
- **Observability**: Use `kubectl get pods,events -n <namespace>` for live pod and operator event status. JSONL logs and stdout logs are collected via OpenTelemetry to operator-chosen backends (e.g. Loki) for dashboards and alerting.
...
- [HIP-1496 - Globally Synchronized Network Upgrade Process](https://hips.hedera.com/#hip-1496)
