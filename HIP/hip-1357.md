...
AccountId should receive block node rewards from the node rewards account `0.0.801` for operating a Tier 1 block node.
...
cost of operating a Tier 1 block node that is distributed to all registerd Tier 1 block node operators. 
...
2. **As a Hiero network governing authority** (e.g., Hedera Council), I want to be able to designate that a network
    node operator Id is eligible for block node rewards fees 1Code has comments. Press enter to view.
...
When setting or changing `block_node_reward_eligible`, the transaction must be signed by the governanace key and the
node admin key.

### Consensus Node Specification

Add a `block_node_reward_eligible` to the node store in the addressbook as described by `node.proto` message
