...
1. For `debug_traceCall` called with `callTracer` the tracer tracks all message frames invoked in the transaction execution.
   ...
   The `actions` field is always an array of bundle objects, each identified by a `bundle_index`. For `debug_traceCall` the array contains exactly one bundle with `bundle_index: 0`. Each bundle contains a `bundle_calls` array of transaction envelopes. Each envelope carries a `transaction_index` (0-based position within the bundle) and a `transaction_calls` array of root `CallAction` objects for that transaction. Each `CallAction` carries a `calls` array of nested sub-calls, allowing arbitrarily deep recursive call trees. The same `CallAction` schema is shared by both `debug_traceCall` and `debug_traceCallMany`.
...
1. For `debug_traceCallMany` called with `callTracer` the tracer tracks all message frames invoked in the transaction execution. It will return basic information about the top level and its nested calls. This tracer supports additional configuration of boolean `only_top_call`, that if set to `true` will return information only for the root top level call, skipping all nested actions.
   ...
   The `actions` field uses the same schema as `debug_traceCall`: an array of bundle objects, each identified by a `bundle_index` (0-based). Each bundle contains a `bundle_calls` array of transaction envelopes. Each envelope carries a `transaction_index` (0-based position within the bundle) and a `transaction_calls` array of root `CallAction` objects for that transaction, in execution order. Each `CallAction` carries a `calls` array of nested sub-calls, enabling arbitrarily deep recursive call trees without any string-encoded index fields.

   Here is a schema visualising the hierarchy of calls:
   ...
   actions[]
     ...
     └── bundle_calls[]
       └── transaction_index
       └── transaction_calls[]
         └── CallAction  (calls, error, from, gas, gas_used, input, output, revert_reason, to, type, value)
           └── calls[]
             └── CallAction  (recursive)
                            ...
                            "calls": [
                                {
                                    ...
                                    "gas": "0x2faf080",
                                    "gas_used": "0x79b6",
                                    "input": "0x80a98232000000000000000000000000d8da6bf26964af9d7eed9e03e53415d37aa96045",
                                    "output": "0x000000000000000000000000000000000000000000000000000000000026009c",
                                    "revert_reason": null,
                                    "to": "0x176211869ca2b568f2a7d4ee941e073a821ee1ff",
                                    "type": "DELEGATECALL",
