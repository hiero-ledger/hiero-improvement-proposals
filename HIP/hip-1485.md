...
The `actions` field is an array of `CallAction` objects. Each `CallAction` carries a `calls` array of nested sub-calls, allowing arbitrarily deep recursive call trees. The same `CallAction` schema is shared by both `debug_traceCall` and `debug_traceCallMany`.
...
The `actions` field uses the same schema as `debug_traceCall` with the difference that here the response will be in a double array. The first index of the array should be matched for a given bundle corresponding to the order of the transactions in the request body. Each `CallAction` carries a `calls` array of nested sub-calls, enabling arbitrarily deep recursive call trees without any string-encoded index fields.
